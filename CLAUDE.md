...
- **Fase 9** ⏳ En curso — App Móvil Expo SDK 56 (`/workspace/simetsa-movil`, JS puro). 9.A ✓ Scaffolding inicial (una sola app conductor+agente, Expo Router file-based). 9.B ✓ Auth global unificada: MovilAuthController (login/logout/me), AuthContext con hasRole/hasAnyRole/can, FCM opcional, redirección post-login por roles, backward compat legacy. 8 tests nuevos, 544 total. 9.C ✓ App conductor completa: 5 tabs (inicio KPIs, tickets, vehículos, multas, perfil), comprar ticket modal, historial, detalles. 9.D ✓ App agente completa: 5 tabs (inicio acciones rápidas, validar placa + iniciar sesión inline Art. 16, registrar infracción, mapa OSM GPS). 9.E ✓ Mapa agente con polígono de zona y posición GPS real-time. 9.F ✓ FCM directo getDevicePushTokenAsync (requiere development build). **9.G** 🔒 Pulido e integración final — pendiente de aprobación. Docs nuevos: `docs/api/movil-auth.md`, `docs/mobile/auth-global.md`. Decisiones: sesion-parqueo.js eliminado (flujo inline en validar-placa), placeholders root eliminados (Expo Router cae al tab navigator).
